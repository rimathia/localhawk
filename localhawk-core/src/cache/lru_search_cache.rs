...
mod tests {
    ...
    fn test_search_cache_basic() {
        let temp_file =
            env::temp_dir().join(format!("localhawk-search-test-{}.json", std::process::id()));
    ...
    fn test_search_cache_entry_limit() {
        let temp_file =
            env::temp_dir().join(format!("localhawk-limit-test-{}.json", std::process::id()));
