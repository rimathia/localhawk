...
impl SetCodesCache {
        ...
        if !force_update {
            ...
            if let Ok(cached) = self.load_from_cache() {
                ...
                if self.is_cache_valid(&cached) {
                    ...
                    warn!(
                        max_age_days = CACHE_DURATION_DAYS,
                        "Set codes disk cache expired"
                    );
                }
            } else {
    ...
    fn load_from_cache(&self) -> Result<CachedSetCodes, ProxyError> {
        if !self.cache_file_path.exists() {
            debug!(
                "Set codes cache file does not exist: {}",
                self.cache_file_path.display()
            );
            return Err(ProxyError::Cache(
                "Set codes cache file not found".to_string(),
            ));
        }
        ...
        let content = fs::read_to_string(&self.cache_file_path).map_err(|e| {
            ProxyError::Cache(format!("Failed to read set codes cache file: {}", e))
        })?;

        let parsed = serde_json::from_str(&content).map_err(|e| {
            ProxyError::Cache(format!("Failed to parse set codes cache file: {}", e))
        })?;
    ...
    fn save_to_cache(&self, set_codes: &ScryfallSetCodes) -> Result<(), ProxyError> {
        ...
        let content = serde_json::to_string_pretty(&cached).map_err(|e| {
            ProxyError::Cache(format!("Failed to serialize set codes cache data: {}", e))
        })?;

        fs::write(&self.cache_file_path, content).map_err(|e| {
            ProxyError::Cache(format!("Failed to write set codes cache file: {}", e))
        })?;

        log::info!(
            "Saved set codes to cache: {}",
            self.cache_file_path.display()
        );
        Ok(())
    }
    ...
    pub fn clear_cache(&self) -> Result<(), ProxyError> {
        if self.cache_file_path.exists() {
            fs::remove_file(&self.cache_file_path).map_err(|e| {
                ProxyError::Cache(format!("Failed to remove set codes cache file: {}", e))
            })?;
        }
        Ok(())
    }
