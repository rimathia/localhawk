...
pub enum BackSide {
    /// Double-faced card back (e.g., Kabira Plateau)
    DfcBack {
        image_url: String,
    ...
    /// (The meld result itself is NOT the physical back, but we show it as the "back" in UI)
    ContributesToMeld {
        meld_result_name: String,      // e.g. "Brisela, Voice of Nightmares"
        meld_result_image_url: String, // Standard-sized image from Scryfall
        meld_partner: String, // e.g. "Bruna, the Fading Light" (always exactly one partner)
        set: String,          // Set of the meld result
    },
}
...
impl Card {
    pub fn from_scryfall_object(
        ...
        let (border_crop, dfc_back_info) = {
            if d.contains_key("image_uris") {
                let front = d["image_uris"]["border_crop"]
                    .as_str()
                    .ok_or_else(|| {
                        ProxyError::InvalidCard("Missing border_crop image".to_string())
                    })?
                    .to_string();
                (front, None)
            } else if d.contains_key("card_faces") {
                let card_faces = d["card_faces"].as_array().ok_or_else(|| {
                    ProxyError::InvalidCard("Invalid card_faces structure".to_string())
                })?;
                ...
                let back_name = card_faces[1]["name"]
                    .as_str()
                    .ok_or_else(|| ProxyError::InvalidCard("Missing back face name".to_string()))?
                    .to_string()
                    .to_lowercase();
        ...
        let meld_info = if d["layout"] == "meld" {
            let all_parts = d["all_parts"].as_array().ok_or_else(|| {
                ProxyError::InvalidCard("Invalid all_parts for meld card".to_string())
            })?;
            ...
            // Find the meld partner (the other meld_part)
            let meld_partner = all_parts
                .iter()
                .filter(|entry| entry["component"] == "meld_part")
                .filter_map(|entry| entry["name"].as_str())
                .map(|s| s.to_lowercase())
                .find(|partner_name| partner_name != &name)
                .ok_or_else(|| ProxyError::InvalidCard("Missing meld partner".to_string()))?;

            if meld_result_name != name {
                Some((meld_result_name, meld_partner))
            } else {
                None
            }
        } else {
        ...
        let back_side = if let Some((back_image, back_name)) = dfc_back_info {
            ...
            Some(BackSide::ContributesToMeld {
                meld_result_name,
                meld_result_image_url: String::new(), // Will be populated during meld resolution
                meld_partner,
                set: set.clone(),
            })
        } else {
    ...
    /// Get all available images for this card for the given face mode
    pub fn get_images_for_face_mode(&self, mode: &crate::DoubleFaceMode) -> Vec<String> {
        match mode {
            crate::DoubleFaceMode::FrontOnly => {
                vec![self.border_crop.clone()]
            }

            crate::DoubleFaceMode::BackOnly => {
                match &self.back_side {
                    Some(BackSide::DfcBack { image_url, .. }) => vec![image_url.clone()],
                    Some(BackSide::ContributesToMeld {
                        meld_result_image_url,
                        ..
                    }) => {
                        if !meld_result_image_url.is_empty() {
                            vec![meld_result_image_url.clone()]
                        } else {
                            vec![self.border_crop.clone()] // Fallback to front if meld not resolved
                        }
                    }
                    None => vec![self.border_crop.clone()], // No back, show front
                }
            }

            crate::DoubleFaceMode::BothSides => {
                let mut images = vec![self.border_crop.clone()];
                match &self.back_side {
    ...
    /// Get the name of the back side (DFC back name or meld result name)
    pub fn back_side_name(&self) -> Option<&str> {
        match &self.back_side {
            Some(BackSide::DfcBack { name, .. }) => Some(name),
            Some(BackSide::ContributesToMeld {
                meld_result_name, ..
    ...
    /// Check if this card contributes to a meld
...
mod tests {
    ...
    fn test_meld_card_parsing() {
        ...
        if let Some(BackSide::ContributesToMeld {
            meld_result_name,
            meld_partner,
            ..
        }) = &card.back_side
        {
            assert_eq!(meld_result_name, "urza, planeswalker");
            assert_eq!(meld_partner, "the mightstone and weakstone");
        } else {
            panic!("Expected ContributesToMeld back side");
        }
    }
